...
public class ACLHomeConfigManipulator {
	...
	public static boolean deleteFieldFromMap(Map<String, Object> map, String field, String functionId, boolean doDecryption) throws Exception {
		...
		if ((map.containsKey("appname") && map.get("appname").equals(securedValue)) || (map.containsKey("name") && map.get("name").equals(securedValue)) || (map.containsKey("id") && map.get("id").equals(securedValue)) || (map.containsKey("text") && map.get("text").equals(securedValue)) ||(map.containsKey("title") && map.get("title").equals(securedValue))) {
