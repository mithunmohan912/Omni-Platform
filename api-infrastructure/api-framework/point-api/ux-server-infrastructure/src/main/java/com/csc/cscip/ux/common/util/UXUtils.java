...
public class UXUtils {
	...
	//This method will escape the special characters. Replace all method will not work in this case as 
	//in case of &amp; we need not to replace & with again &amp;.
	public static String escapeSpecialChar(String sqlStmt){
		...
		for(int i =0;i<sqlStmt.length();i++)
		{
			...
			if(sqlStmt.charAt(i)=='&')
			{
				if(i==sqlStmt.length()-1)
				{
					sqlStmt = sqlStmt + "amp;" ;
				}	
				else
				{	
					String tempstr = sqlStmt.substring(i+1, i+4);
					if(!tempstr.equalsIgnoreCase("amp;") && !tempstr.equalsIgnoreCase("lt;") && !tempstr.equalsIgnoreCase("gt;") && !tempstr.equalsIgnoreCase("quot;") )
					{
						sqlStmt = sqlStmt.substring(0,i+1) + "amp;" + sqlStmt.substring(i+1);
					}	
			...
			if(sqlStmt.charAt(i)=='>')
			{
				sqlStmt = sqlStmt.substring(0,i) + "&gt;" + sqlStmt.substring(i+1);
			}	
			if(sqlStmt.charAt(i)=='<')
			{
				sqlStmt = sqlStmt.substring(0,i) + "&lt;" + sqlStmt.substring(i+1);
				...
				if (datePart[2].length() == 2) { // MM/DD/YY
					...
					mm = datePart[0];
							...
							if(datePart[1].length() != 2){
								dd = StringUtils.fill(datePart[1],"0" ,2); 
							}
							else{
								dd=datePart[1];
							} 
